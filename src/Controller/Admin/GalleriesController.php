...
class GalleriesController extends AppController
{
    ...
    public function index($clearFilter = null)
    {
		...
		$this->set('title', __('Browse the') . ': ' . __('Galleries'));
		...
		$this->paginate['Galleries']['page'] 	= $page;
		
		if($sort !== null && $direction !== null){
			$this->paginate['Galleries']['order'] 	= [$sort => $direction];
		}else{
			$this->paginate['Galleries']['order'] 	= [$this->defaultOrder];
		}
		...
		if($search !== ''){
			...
			$query = $this->Galleries->find()->where($conditions);
		}
		// ############################# /.QUERY ###########################################
		...
		// ############################# PAGINATE ############################################
		try {
			$this->paginate['Galleries']['limit'] = $this->config['paginate_limit'];
			$this->paginate['Galleries']['maxLimit'] = $this->config['paginate_limit'];
    ...
    public function view($id = null)
    {
		...
		try {
			$gallery = $this->Galleries->get((int) $id, contain: ['Photos']);
		} catch (\Cake\Datasource\Exception\RecordNotFoundException $exeption) {
			$this->Flash->warning(__($exeption->getMessage()), ['plugin' => 'JeffAdmin5']);
			return $this->redirect(['action' => 'index']);
		}

		$this->set('title', __('View the') . ': ' . __('gallery') . ' ' . __('record'));
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		$name = $gallery->name;
		$this->set(compact('gallery', 'id', 'name'));
    }
    ...
    public function add()
    {
		...
		$this->set('title', __('Add new') . ': ' . __('gallery') . ' ' . __('record'));
        ...
        if ($this->request->is('post')) {
            ...
            if ($this->Galleries->save($gallery)) {
                //$this->Flash->success(__('The gallery has been saved.'), ['plugin' => 'JeffAdmin5']);
                $this->Flash->success(__('The save has been: OK'), ['plugin' => 'JeffAdmin5']);
				$this->session->write('Layout.' . $this->controller . '.LastId', $gallery->id);
            ...
            //$this->Flash->error(__('The gallery could not be saved. Please, try again.'), ['plugin' => 'JeffAdmin5']);
    ...
    public function edit($id = null)
    {
		...
		try {
			$gallery = $this->Galleries->get((int) $id, contain: []);
		} catch (\Cake\Datasource\Exception\RecordNotFoundException $exeption) {
			$this->Flash->warning(__($exeption->getMessage()), ['plugin' => 'JeffAdmin5']);
			return $this->redirect(['action' => 'index']);
		}

		$this->set('title', __('Edit the') . ': ' . __('gallery') . ' ' . __('record'));
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
    ...
    public function delete($id = null)
    {
		...
		try {
			$gallery = $this->Galleries->get((int) $id);
		} catch (\Cake\Datasource\Exception\RecordNotFoundException $exeption) {
			$this->Flash->warning(__($exeption->getMessage()), ['plugin' => 'JeffAdmin5']);
			return $this->redirect(['action' => 'index']);
		}
		...
		if ($this->Galleries->delete($gallery)) {
			...
			//$this->Flash->success(__('The gallery has been deleted.'), ['plugin' => 'JeffAdmin5']);
			$this->Flash->success(__('The has been deleted.'), ['plugin' => 'JeffAdmin5']);
		} else {
			//$this->Flash->error(__('The gallery could not be deleted. Please, try again.'), ['plugin' => 'JeffAdmin5']);
