						...
						<div class="card mb-3">
							...
							<div class="card-header">
								<div class="float-start">
									<h3><i id="card-icon" class="fa fa-eye fa-spin"></i> <?= __('View') ?>: <?= __('Post') ?></h3>
...
<?php } ?>
											<div class="row"><!-- 1. -->
												<label class="col-sm-2 col-form-label p-1 text-start text-sm-end"><?= __('My User') ?>:</label>
												<div class="col-sm-10 p-1 link">
													<?= $post->hasValue('my_user') ? $this->Html->link($post->my_user->first_name . ' ' . $post->my_user->last_name, ['controller' => 'MyUsers', 'action' => 'view', $post->my_user->id]) : '' ?><span class="external-link-icon"><i class="fa fa-external-link" aria-hidden="true"></i></span>
												</div>
											</div>
											<div class="row"><!-- 2. -->
											...
											<div class="row"><!-- 5. -->
												<label class="col-sm-2 col-form-label p-1 text-start text-sm-end"><?= __('Show In Main Page') ?>:</label>
												<div class="col-sm-10 p-1">
													<?= $post->show_in_main_page ? '<i class="fa fa-check-square boolean-yes" aria-hidden="true"></i>' : '<i class="fa fa-square boolean-no" aria-hidden="false"></i>' ?>
...
<?php /*
											<div class="row"><!-- 6. -->
												<label class="col-sm-2 col-form-label p-1 text-start text-sm-end"><?= __('More') ?>:</label>
												<div class="col-sm-10 p-1">
													<?= $this->Text->autoParagraph(h($post->more)) ?>
...
*/ ?>
										...
										<!-- TAB for: More text field -->
										<div class="tab-pane fade" id="tabPanelMore" role="tabpanel" aria-labelledby="tabMore" tabindex="0">
											<div class="row">
												<div class="col-sm-12">
													<div class="row">
														<div id="readMoreMore" class="col-sm-12 p-2 text read-more">
															<?= $this->Text->autoParagraph($post->more) ?>
